...
//			LOG_DEBUG_STR("Rcuallocation: " << RCUallocation);
			...
			for (int rcu = 0; rcu < MAX_RCUS; rcu++) {
				...
				for (int element = 0; element < N_HBA_ELEM_PER_TILE; ++element) {
                    ...
                    int delayStepNr = static_cast<int>((itsHBAdelays(rcu, element) & 0x7c) >> 2);  // set actual delay

                    // search for next step
                    for (int i = 0; i < maxStepNr; ++i) {
                        if (fabs(delay - itsDelaySteps(i)) <= 0.2E-9) {
                            delayStepNr = i;
                            break;
                        }
                    }

                    if (fabs(delay - itsDelaySteps(delayStepNr)) > 0.6E-9) {
                        LOG_INFO_STR("delay diff > 0.75E-9, calculate approximate delaynr");
                        delayStepNr = static_cast<int>(fabs(delay) / 0.5E-9);
                    }
                    ...
                    if (itsDiffHBArotations) {
                        if ((rcu == 0) && (rcu%2 == 0)) {
                            LOG_INFO_STR("HBA-0 delay, element="<<element  <<", requested delay="<<delay <<", real delay="<<itsDelaySteps(delayStepNr));
                        }
                        if ((rcu == 48) && (rcu%2 == 0)) {
                            LOG_INFO_STR("HBA-1 delay, element="<<element  <<", requested delay="<<delay <<", real delay="<<itsDelaySteps(delayStepNr));
                        }
                    }
                    else {
                        if ((rcu == 0) && (rcu%2 == 0)) {
                            LOG_INFO_STR("HBA delay, element="<<element  <<", requested delay="<<delay <<", real delay="<<itsDelaySteps(delayStepNr));
                        }
                    }

					// bit1=0.25nS(not used), bit2=0.5nS, bit3=1nS, bit4=2nS, bit5=4nS, bit6=8nS
					itsHBAdelays(rcu, element) = (delayStepNr << 2) + (1 << 7); // assign
				} // for element
			} // rcus
			++pIter;
		} // pointings
