...
static unique_ptr<PortBroker> pbInstance;
...
void PortBroker::start() {
  itsThread.reset(new Thread(this, &PortBroker::serverLoop, "PortBroker", "[PortBroker] "));
}
...
void PortBroker::serverLoop()
{
  while(!itsDone) {
    ...
    unique_ptr<FileDescriptorBasedStream> clientStream(detach());
...
PortBroker::ConnectedClient PortBroker::waitForClient( const string &resource, bool prefix, time_t deadline ) {
  ...
  while(!itsDone) {
    ...
    if (it != itsRequestMap.end()) {
      unique_ptr<FileDescriptorBasedStream> serverStream(it->second);
...
PortBroker::ServerStream::ServerStream( const string &resource, bool prefix, time_t deadline )
{
  ...
  unique_ptr<FileDescriptorBasedStream> stream(client.stream);
