  ...
  namespace Cobalt
  {
    ...
    struct ObservationSettings {
      ...
      struct AntennaFieldName {
        ...
        bool operator==(const AntennaFieldName &other) const {
      ...
      struct AntennaField {
        ...
        // Allow comparing by name, useful for std::find.
        bool operator==(const std::string &other) const { return name == other; }
        bool operator==(const AntennaFieldName &other) const { return name == other.fullName(); }
      };
      ...
      std::vector<struct AntennaFieldName> antennaFieldNames;
      ...
      struct Correlator {
        ...
        struct File {
          ...
          // Where to create the (meta)data, before moving them to their
          // final location. If empty, directly create the (meta)data in
          // their final location.

          // data is moved through a single staging dir, which needs to
          // be on the same mount point as the final location.
          //
          // metaData is moved through 2 staging dirs. The files are
          // created in stagingDir1, moved to stagingDir2 to move to
          // the target mount point, and then renamed to their final location.
          std::string dataStagingDirTargetFS; // stage metadata here on target FS
          std::string metaDataStagingDirCreateFS; // create metadata here
          std::string metaDataStagingDirTargetFS; // stage metadata here on target FS
        };
      ...
      expandAntennaFieldNames(const std::vector<std::string> &stations,
