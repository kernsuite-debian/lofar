...
//TODO				(GET_CONFIG("CalServer.N_SUBBANDS", i) >=
...
// TODO what about analogue(-only) beams???
	...
	int		activationTime = _idealStartTime( Timestamp::now().sec(),
                                              ptEvent.pointing.time(),
                                              HBA_MIN_INTERVAL,
                                              itsLastHBACalculationTime + itsHBAUpdateInterval,
                                              HBA_MIN_INTERVAL,
                                              itsHBAUpdateInterval);
	...
	if (ptEvent.pointing.duration() && timeShift) {
		ptEvent.pointing.setDuration(ptEvent.pointing.duration());
    ...
    itsAnaHeartbeat->cancelAllTimers();
    itsAnaHeartbeat->setTimer(activationTime - Timestamp::now().sec(), 0, itsHBAUpdateInterval, 0);	// restart analog beam hartbeat
...
int	BeamServer::_idealStartTime (int now, int t1, int d1, int t2, int d2, int p2) const
{
    ...
    if (t1start <= (t2 - p2 + d2))  // if it is to fast after previous update
        t1start = t2 - p2 + d2 + 1;
