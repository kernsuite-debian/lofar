//# LofarCubeSkyEquation.cc: Implementation of Cube Optimized Sky Equation classes
//# Copyright (C) 2007
...
LofarCubeSkyEquation::LofarCubeSkyEquation(SkyModel& sm, ROVisibilityIterator& vi, FTMachine& ft,
                                 ComponentFTMachine& cft, Bool noModelCol)
...
void LofarCubeSkyEquation::init(FTMachine& ft){
  ...
  if(ft.name() == "MosaicFT"){
    ...
    for (Int k=1; k < (nmod); ++k){
     ...
     ft_=new LOFAR::LofarFTMachine(static_cast<LOFAR::LofarFTMachine &>(ft));
     ift_=new LOFAR::LofarFTMachine(static_cast<LOFAR::LofarFTMachine &>(ft));
     //     ift_=ft_;
     ftm_p[0]=ft_;
     iftm_p[0]=ift_;
...
void LofarCubeSkyEquation::makeMosaicPSF(PtrBlock<TempImage<Float> * >& psfs){
...
void LofarCubeSkyEquation::getCoverageImage(Int model, ImageInterface<Float>& im){
...
void LofarCubeSkyEquation::fixImageScale()
{
