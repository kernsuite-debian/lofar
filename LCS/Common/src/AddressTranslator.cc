...
//#
//# $Id: AddressTranslator.cc 34754 2016-06-20 10:56:00Z schaap $
...
namespace LOFAR
{
  ...
  namespace
  {
    ...
    typedef std::map<bfd_vma, SymbolTablePtr> SymbolTableMap;
    ...
    for (int i = 0; i < size; i++) {
      ...
      if(it == theSymbolTableMap().end()) {
        it = theSymbolTableMap().insert(
          std::make_pair(base_addr, 
                         SymbolTablePtr(new SymbolTable(bfdFile)))).first;
      }
