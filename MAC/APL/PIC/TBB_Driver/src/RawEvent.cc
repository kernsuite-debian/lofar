...
GCFEvent::TResult RawEvent::dispatch(GCFTask& task, GCFPortInterface& port)
{
	...
	switch(buf.opcode) {
		...
		case oc_DISPMEAS:
			buf.event.signal = TP_DISPMEAS_ACK;
			buf.event.length = sizeof(uint32) * 2;
			break;

		case oc_READBAND:
			buf.event.signal = TP_READBAND_ACK;
			buf.event.length = sizeof(uint32) * 3;
			break;

		case oc_STOPTIMED:
			buf.event.signal = TP_STOPTIMED_ACK;
			buf.event.length = sizeof(uint32) * 2;
			break;

        case oc_SINGLE_PACKET_CEP_TEST:
            buf.event.signal = TP_SINGLE_PACKET_CEP_TEST_ACK;
            buf.event.length = 8;
            break;
