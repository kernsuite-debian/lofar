...
def saveData(filename, data):
    with open(filename, 'wb') as f:  # binary: write as-is; no conv!
        f.write(data)
...
def processMessages(receiver, matchPrefix, execPath, msgSaveDir):
    while True:
        ...
        try:
            ...
            logger.error('Failed to parse or retrieve node from XML message: %s', exc)
    ...
    while True:
        ...
        try:
            ...
            if timeout == 0.1:
                ...
                break
            logger.error('%s; sleeping %.1f seconds', exc, timeout)
            sleep(timeout)
...
def registerCmdOptions(parser):
    ...
    if daemon_exc is not None:
        daemon_help += ' (N/A: ImportError: ' + str(daemon_exc) + ')'
...
def checkArgs(parser, options, leftOverArgs):
    ...
    if options.daemonize and daemon_exc is not None:
        parser.error('--daemon (-d) is N/A: ImportError: ' + str(daemon_exc))
