...
GCFEvent::TResult beamctl::sendPointings(GCFEvent& event, GCFPortInterface& port)
{
    ...
    case IBS_POINTTOACK: {
        ...
        if (ack.status != IBS_Protocol::IBS_NO_ERR) {
            cerr << "Error: " << errorName(ack.status) << endl;
            if (ack.status == IBS_Protocol::IBS_UNKNOWN_BEAM_ERR) {
                cerr << "       Possible wrong bitmode ?" << endl;
            }
            TRAN(beamctl::final);
        } else {
...
bool beamctl::checkOptions()
{
    ...
    if (itsBand == BAND_UNDEFINED) {
        if (itsRCUmode == -1) {
            cerr << "Error: not a valid band specified." << endl;
            return (false);
        }
...
bool beamctl::parseOptions(int  myArgc, char** myArgv)
{
    ...
    while (true) {
        int option_index = 0;
        int c = getopt_long(myArgc, myArgv, "a:r:B:m:A:D:s:b:ch", long_options, &option_index);
        if (c == -1) {
            break;
        }
