...
namespace casacore{
  ...
  //
  template
  void LofarVisibilityResampler::DataToGridImpl_p(Array<DComplex>& grid, LofarVBStore& vbs, 
					     const Bool& dopsf,  Matrix<Double>& sumwt) __restrict__;
  template
  void LofarVisibilityResampler::DataToGridImpl_p(Array<Complex>& grid, LofarVBStore& vbs, 
  					     const Bool& dopsf,  Matrix<Double>& sumwt) __restrict__;
  ...
  template <class T>
  void LofarVisibilityResampler::DataToGridImpl_p(Array<T>& grid,  LofarVBStore& vbs, const Bool& dopsf,
                         Matrix<Double>& sumwt)
  {
    ...
    for(Int sel_idx=rbeg; sel_idx  < rend; sel_idx++){          // For all rows
      Int irow;
      if (vbs.selection().empty()) 
      {
         irow = sel_idx;
      }
      else 
      {
         irow = vbs.selection()(sel_idx);
      }
  ...
  //
  void LofarVisibilityResampler::GridToData(LofarVBStore& vbs, const Array<Complex>& grid)
  ...
  //     phasor=Complex(1.0);
  // }
  void LofarVisibilityResampler::ComputeResiduals(LofarVBStore& vbs)
  {
