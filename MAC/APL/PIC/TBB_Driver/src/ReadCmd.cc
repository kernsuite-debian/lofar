...
void ReadCmd::saveTbbEvent(GCFEvent& event)
{
	...
	itsTimestamp.set(tbb_event.nstimestamp);
	itsTimeBefore.set(tbb_event.nstimebefore);
	itsTimeAfter.set(tbb_event.nstimeafter);
	LOG_DEBUG_STR(formatString("itsTimestamp  =  %ld seconds  %ld nseconds", itsTimestamp.sec(), itsTimestamp.nsec()));
    LOG_DEBUG_STR(formatString("itsTimeBefore =  %ld seconds  %ld nseconds", itsTimeBefore.sec(), itsTimeBefore.nsec()));
    LOG_DEBUG_STR(formatString("itsTimeAfter  =  %ld seconds  %ld nseconds", itsTimeAfter.sec(), itsTimeAfter.nsec()));
    nextChannelNr();
	...
	if (event.signal == F_TIMER) {
		...
		if (tp_ack.status != 0) {
			...
			if (itsStage == 0) {
				...
				NsTimestamp lastSampleTime((double)itsLastSavedSecond + ((itsLastSavedSampleNr + itsLastSavedNrOfSamples) * sampletime));
                ...
                LOG_DEBUG_STR(formatString("firstSampleTime=  %ld seconds  %ld nseconds", firstSampleTime.sec(), firstSampleTime.nsec()));
                LOG_DEBUG_STR(formatString("lastSampleTime =  %ld seconds  %ld nseconds", lastSampleTime.sec(), lastSampleTime.nsec()));
                LOG_DEBUG_STR(formatString("startTimestamp =  %ld seconds  %ld nseconds", startTimestamp.sec(), startTimestamp.nsec()));
                LOG_DEBUG_STR(formatString("stopTimestamp  =  %ld seconds  %ld nseconds", stopTimestamp.sec(), stopTimestamp.nsec()));
				#endif
