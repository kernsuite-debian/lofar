...
void navFunct_fillHardwareTree() {
  ...
  if (dynlen(g_stationList) > 1 ) {
    ...
    if (g_stationList[1]+":" == CEPDBName) {
      ...
      // add PowerUnits
      if (dynlen(g_powerUnitList) > 0) {
        for (int i = 1; i <= dynlen(g_powerUnitList); i++) {
          dp = station+":LOFAR_PIC_POWEC"+g_powerUnitList[i];
          dynAppend(result,connectTo+",POWEC"+g_powerUnitList[i]+","+dp);
        }
        lvl="PowerUnit";
      }
