...
GCFEvent::TResult TrigInfoCmd::ack(GCFEvent& e)
{
    ...
    if (ack.status_mask == TBB_SUCCESS) {
        cout << formatString(" %2d  time_sec   : %u",getRcu(),ack.nstimestamp.sec()) << endl;
        cout << formatString("      time_nsec  : %u",ack.nstimestamp.nsec()) << endl;
...
// READBAND
ReadBandCmd::ReadBandCmd(GCFPortInterface& port):
    Command(port),
    itsStage(0),
    itsChannel(0),
    itsSubband(0),
    ...
    itsPeriod(0)
{
    std::cout << "\n== TBB ==============  transfer ALERT spectral data to "
        "CEP for selected slices ====\n\n";
}

void ReadBandCmd::send()
{
    TBBReadbandEvent event;
    event.boardmask = getBoardMask();
    event.channel = itsChannel;
    event.subband = itsSubband;
    event.time = itsTime;
    event.slicenr = itsSlicenr;
    event.period = itsPeriod;

    itsPort.send(event);
    itsPort.setTimer(DELAY);
}

GCFEvent::TResult ReadBandCmd::ack(GCFEvent& e)
{
    TBBReadbandAckEvent ack(e);
    std::cout << "TBB  Info\n"
        "---  -------------------------------------------------------\n";
    for(int bnr(0); bnr < getMaxSelections(); ++bnr)
    {
        if(isSelected(bnr) == true)
        {
            if(ack.status_mask[bnr] == TBB_SUCCESS)
            {
                std::cout << formatString(" %2d  read band:  "
                    "channel %u, "
                    "subband %u, "
                    "time %u[s], "
                    "slice #%u, "
                    ...
                    "readstatus %u, "
                    "total_nof_samples %u, "
                    "nof_samples_next_frame %u, "
                    "mp_index %u, "
                    "interval_start_address %u, "
                    "buffer_stop_seconds %u, "
                    "buffer_stop_slices %u, "
                    "not_used %u, "
                    "writer_status %u, ",
                    bnr,
                    itsChannel,
                    ...
                    itsPeriod,
                    ack.readstatus[bnr],
                    ack.total_nof_samples[bnr],
                    ack.nof_samples_next_frame[bnr],
                    ack.mp_index[bnr],
                    ack.interval_start_address[bnr],
                    ack.buffer_stop_seconds[bnr],
                    ack.buffer_stop_slices[bnr],
                    ack.not_used[bnr],
                    ack.writer_status[bnr])
                    << "\n";
            }
            else
            {
                std::cout << formatString(" %2d  %s",bnr,
                    getDriverErrorStr(ack.status_mask[bnr]).c_str())
                    << "\n";
            }
        }
    }
...
// STOPTIMED
StopTimedCmd::StopTimedCmd(GCFPortInterface& port):
    Command(port),
    itsStage(0),
    itsTime(0),
    itsSlicenr(0)
{
    std::cout << "\n== TBB ==============  setting the stop time for selected "
        "slices ====\n\n";
}

void StopTimedCmd::send()
{
    TBBStoptimedEvent event;
    event.boardmask = getBoardMask();
    event.time = itsTime;
    event.slicenr = itsSlicenr;
...
GCFEvent::TResult StopTimedCmd::ack(GCFEvent& e)
{
    TBBStoptimedAckEvent ack(e);
    std::cout << "TBB  Info\n"
        "---  -------------------------------------------------------\n";
    for(int bnr(0); bnr < getMaxSelections(); ++bnr)
    {
        if(isSelected(bnr) == true)
        {
            if(ack.status_mask[bnr] == TBB_SUCCESS)
            {
                std::cout << formatString(" %2d  stop timed:  "
                    "time %u[s], slice #%u", bnr, itsTime, itsSlicenr)
                    << "\n";
            }
            else
            {
...
// DISPMEAS
DispMeasCmd::DispMeasCmd(GCFPortInterface& port):
    Command(port),
    itsStage(0),
    itsDm(0)
{
    std::cout << "\n== TBB ==============  setting the dispersion measure "
        "value ====\n\n";
}

void DispMeasCmd::send()
{
    TBBDispmeasEvent event;
    event.boardmask = getBoardMask();
    event.dm = itsDm;
...
GCFEvent::TResult DispMeasCmd::ack(GCFEvent& e)
{
    TBBDispmeasAckEvent ack(e);
    std::cout << "TBB  Info\n"
        "---  -------------------------------------------------------\n";
    for(int bnr(0); bnr < getMaxSelections(); ++bnr)
    {
        if(isSelected(bnr) == true)
        {
            if(ack.status_mask[bnr] == TBB_SUCCESS)
            {
                std::cout << formatString(" %2d  dispersion measure value %u",
                    bnr, itsDm)
                    << "\n";
            }
            else
...
// SINGLE_PACKET_CEP_TEST
SinglePacketCepTestCmd::SinglePacketCepTestCmd(GCFPortInterface& port):
    Command(port)
{
...
void SinglePacketCepTestCmd::send()
{
    TBBSinglePacketCepTestEvent event;
    event.boardmask = getBoardMask();
...
GCFEvent::TResult SinglePacketCepTestCmd::ack(GCFEvent& e)
{
    TBBSinglePacketCepTestAckEvent ack(e);
    std::cout << "Single packet CEP test\n"
        "---  ----\n";
    for(int bnr(0); bnr < getMaxSelections(); ++bnr)
    {
        if(isSelected(bnr) == true)
...
// UDP
UdpCmd::UdpCmd(GCFPortInterface& port):
    Command(port)
{
...
// CEP STATUS
CepStatusCmd::CepStatusCmd(GCFPortInterface& port) : Command(port)
{
    cout << endl
        << "== TBB ==================================== CEP status ===="
        << endl << endl;
...
//-----------------------------------------------------------------------------
GCFEvent::TResult CepStatusCmd::ack(GCFEvent& e)
{
    TBBCepStatusAckEvent ack(e);
    for(ssize_t bnr(0); bnr < getMaxSelections(); ++bnr)
    {
        if(isSelected(bnr) == true)
        {
            if(ack.status_mask[bnr] == TBB_SUCCESS)
            {
                cout << formatString(" %2d  %8d\n\n",
                    bnr, ack.pages_left[bnr])
                    << endl;
            }
            else
            {
                cout << formatString(" %2d  %s\n\n",
                    bnr, getDriverErrorStr(ack.status_mask[bnr]).c_str())
                    << endl;
            }
        }
    }
...
GCFEvent::TResult ReadAllCmd::ack(GCFEvent& e)
{
    ...
    if (e.signal == TBB_STOP_ACK) {
        ...
        if (itsStage == 1) {
            ...
            sampleNr = ack.data[1];
            //cout << formatString("board %d, rcu %-3d, last recorded time %u seconds and sample number %u",
            ...
            cout << formatString("board %d, rcu %-3d, last recorded time=%u seconds, sample number=%lu and sample-freq=%dMHz",
                                        itsBoard, itsRcu, secondsTime, sampleNr,sampleFreq) << endl;
            cout << formatString("         time       = %u seconds, %u nseconds",
                                 itsTime.sec(), itsTime.nsec()) << endl;
            cout << formatString("         timebefore = %u seconds, %u nseconds",
                                 itsTimeBefore.sec(), itsTimeBefore.nsec()) << endl;
            cout << formatString("         timeafter  = %u seconds, %u nseconds",
...
TestDdrCmd::TestDdrCmd(GCFPortInterface& port) : Command(port),
    itsCmdStage(0),itsAddrLines(28), itsMp(0), itsAddr(0), itsAddrLine(2), itsTestPatern(1),
...
{
    if ((itsCmdStage == 1)&&(itsMp == 0)&&(itsAddrLine == 2)) {
        cout << endl;
        cout << "Mp  Addr                                                             " << endl;
        cout << "Nr  Line   Written     Read         Info                         " << endl;
        cout << "--  -----  ----------  ----------   -----------------------------" << endl;
        cout << " Skipping first two addresslines because of 4 word alignment" << endl;
        cout << endl;
    }
    ...
    switch (itsCmdStage) {
        case 0: {
            ...
            if (ack.status_mask[getBoard()] == TBB_SUCCESS) {
                ...
                // word adressing (64 bit), memsize one MP in words = memsize MP / 8
                // from number of pages to memsize one MP in words,
                // memsize one MP in words = (pages * 2048) / 4 / 8 = pages on board * 64
                // total adrress lines = log(pages on board * 64) / log(2)

                double mpMemorySize = ((double)ack.npages[getBoard()] * 2048.) / (1024. * 1024. * 1024.) / 4.;
                cout << "Memory size 1 MP = " << mpMemorySize << " Gb" << endl;
                itsAddrLines = static_cast<uint32>(log10(ack.npages[getBoard()] * 64.) / log10(2.));
...
void TBBCtl::commandHelp(int level)
{
    ...
    if (level == 3)
    {
        cout << " tbbctl --testddr=board                                                # ddr memory test, address and data lines\n"
    ...
    cout << " tbbctl --imageinfo=board                                              # read info from all images on board\n"
    ...
    if (level == 3)
    {
        cout << " tbbctl --templimits=high,low [--select=<boardset>]                         # set temp limts for fan control\n";
    }
...
GCFEvent::TResult TBBCtl::docommand(GCFEvent& e, GCFPortInterface& port)
{
    ...
    switch (e.signal) {
        ...
        case TBB_DISPMEAS_ACK:
        case TBB_READBAND_ACK:
        case TBB_STOPTIMED_ACK:
        case TBB_SINGLE_PACKET_CEP_TEST_ACK:
        {
            itsServerPort.cancelAllTimers();
...
Command* TBBCtl::parse_options(int argc, char** argv)
{
    ...
    while(1) {
        static struct option long_options[] = {
            ...
            {"readband", required_argument, 0, 'R'},
            {"stoptimed", required_argument, 0, 'S'},
            {"dispmeas", required_argument, 0, 'T'},
            {"singlepacketceptest", no_argument, 0, 'U'},
            {"udp", no_argument, 0, 'V'},
            {"cepstatus", no_argument, 0, 'W'},
            { 0,            0,                 0,  0 },
        };

        int option_index = 0;
        int c = getopt_long(argc, argv,
            "a:bcdefghij:k:l:m::n:o:p:qrst:uv:wx:y:zAB:C:D:E:F:G:H:I:J:K:LMN:R:S:T:UVW",
        ...
        switch (c) {
            ...
            case 'J': {    // --readr
                ...
                if (optarg) {
                    ...
                    if (numitems < 4 || numitems == EOF || board < 0 || board >= itsMaxBoards || mp > 3 || pid > 8 || regid > 7) {
                        cout << "Error: invalid read register value. Should be of the format" << endl;
                        cout << "       '--readreg=board, mp, pid, regid'" << endl;
                        cout << "       board=0.." << (itsMaxBoards - 1) << ",  mp=0..3,  pid=0..8,  regid=0..7" << endl;
                        exit(EXIT_FAILURE);
                    }
            ...
            case 'R':
            {
                ...
                ReadBandCmd* cmd(new ReadBandCmd(itsServerPort));
                command = cmd;
                if(optarg)
                {
                    int32 board(0);
                    ...
                    int numitems = sscanf(optarg, "%u,%u,%u,%u,%u,%u",
                        &board, &channel, &subband, &time, &slicenr, &period);
                    if((numitems != 6)
                    || (numitems == EOF)
                    || (board < 0)
                    || (board >= itsMaxBoards))
                    {
                        std::cout << "Error: invalid read band parameter(s).  "
                            "The parameters should have this format:\n"
                            "       '--readband=board, channel, subband, time[s], "
                            "slice#, period[1e-3s]'\n";
                    ...
                    cmd->setSubband(subband);

                    cmd->setTime(time);
                    cmd->setSlicenr(slicenr);
            ...
            case 'S':
            {
                ...
                StopTimedCmd* cmd(new StopTimedCmd(itsServerPort));
                command = cmd;
                if(optarg)
                {
                    int32 board(0);
                    ...
                    int numitems = sscanf(optarg, "%u,%u,%u",
                        &board, &time, &slicenr);
                    if((numitems != 3)
                    || (numitems == EOF)
                    || (board < 0)
                    || (board >= itsMaxBoards))
                    {
                        std::cout << "Error: invalid stop timed parameter(s).  "
                            "The parameters should have this format:\n"
            ...
            case 'T':
            {
                ...
                DispMeasCmd* cmd(new DispMeasCmd(itsServerPort));
                command = cmd;
                if(optarg)
                {
                    int32 board(0);
                    ...
                    int numitems = sscanf(optarg, "%u,%lf", &board, &dm);
                    if((numitems != 2)
                    || (numitems == EOF)
                    || (board < 0)
                    || (board >= itsMaxBoards))
                    {
                        std::cout << "Error: invalid disp meas parameter(s).  "
                            "The parameters should have this format:\n"
                            "       '--stoptimed=board, DM'\n";
                    ...
                    cmd->setDm(static_cast< uint32 >(dm * 65536.0));
            ...
            case 'U':
            {
                ...
                command = new SinglePacketCepTestCmd(itsServerPort);
                command->setCmdType(BOARDCMD);
            }
            break;
            ...
            case 'W':
            {
                ...
                command = new CepStatusCmd(itsServerPort);
                command->setCmdType(BOARDCMD);
            } break;
...
std::list<int> TBBCtl::strtolist(const char* str, int max)
{
    ...
    while(start) {
        ...
        if (val >= max || val < 0) {
            cout << formatString("Error: value %d out of range",val) << endl;
            resultset.clear();
            return(resultset);
        }
