...
int main() {
  ...
  unique_ptr<IntToFloatKernel> kernel(factory.create(stream, devSampledData, devConvertedData));
  ...
  for (size_t idx =0; idx < nSampledData; ++idx) {
    int fftshift = (idx/2) % 2 ? -1 : +1; // idx alternates real and imag
    float expected = -127 * 16 * fftshift;
    if (samplesFloat[idx] != expected)
    {
