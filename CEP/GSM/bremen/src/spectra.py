...
def _verify_versions(a, b):
    """
    Check that version a is lower than version b.
    >>> _verify_versions('1.0', '2.0')
    True
    >>> _verify_versions('1.4', '2.0')
    True
    >>> _verify_versions('1.4', '1.3.2')
    False
    """
...
if _verify_versions('1.4.0', numpy.__version__):
    ...
    def polyval(x, args):
        """
        Introduce polyval function missing in Numpy 1.3.
        """
...
select case when last_update_date > last_spectra_update_date
 ...
 where runcatid = {0};""".format(runcatid))
        ...
        if not self.need_update:
            for order in range(int(result[1])):
...
select %s f.freq_central), %s rf.wm_f_int), rf.avg_weight_f_int
 ...
 where f.freqbandid = rf.band
   ...
   and rf.stokes = 'I'""" % (func, func, runcat_id))
        ...
        sp_update = ','.join(['spectral_index_%s = %s' %
                                       (x, self.args[x]) for x in range(sp_power)])
        self.conn.execute("""
