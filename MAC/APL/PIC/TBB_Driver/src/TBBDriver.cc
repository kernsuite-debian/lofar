...
GCFEvent::TResult TBBDriver::setup_state(GCFEvent& event, GCFPortInterface& port)
{
    ...
    switch(event.signal) {
        ...
        case TP_VERSION_ACK:
        {
            int board = TS->port2Board(&port); // get board nr
            itsBoard[board].cancelAllTimers();
            TPVersionAckEvent ack(event);
            LOG_INFO_STR(formatString("Received VersionAck from boardnr[%d], status=%d", board, ack.status));
            if (ack.status == TBB_SUCCESS) {
                TS->setImageSwVersion(board, ack.tpswversion);
                TS->setBoardState(board, swVersionChecked);
            }
...
GCFEvent::TResult TBBDriver::busy_state(GCFEvent& event, GCFPortInterface& port)
{
    ...
    switch(event.signal) {
        ...
        case TP_DISPMEAS_ACK:
        case TP_READBAND_ACK:
        case TP_STOPTIMED_ACK:
        case TP_SINGLE_PACKET_CEP_TEST_ACK:
        case TP_UDP_ACK:
        {
                ...
                if ((ack.resetflag == 0) || ((TS->activeBoardsMask() & (1 << boardnr)) == 0)) {
                    ...
                    if (TS->getFreeToReset(boardnr)) {
                        if (0 != TS->getDefaultImageNr()) {
                            ...
                            TS->setImageSwVersion(boardnr, 0);
...
bool TBBDriver::addTbbCommandToQueue(GCFEvent& event, GCFPortInterface& port)
{
    switch(event.signal)
    {
        ...
        case TBB_TEMP_LIMIT:
        case TBB_CEP_STORAGE:
        case TBB_SINGLE_PACKET_CEP_TEST:
        case TBB_UDP:
        {
...
bool TBBDriver::SetTbbCommand(unsigned short signal)
{
    ...
    switch(signal)
    {
        ...
        case TBB_SINGLE_PACKET_CEP_TEST:  {
            SinglePacketCepTestCmd* itsCmd(new SinglePacketCepTestCmd);
            itsCmdHandler->setTpCmd(itsCmd);
        } break;
        ...
        case TBB_DISPMEAS:
        {
            DispMeasCmd* itsCmd(new DispMeasCmd);
            itsCmdHandler->setTpCmd(itsCmd);
        } break;

        case TBB_READBAND:
        {
            ReadBandCmd* itsCmd(new ReadBandCmd);
            itsCmdHandler->setTpCmd(itsCmd);
        } break;

        case TBB_STOPTIMED:
        {
            StopTimedCmd* itsCmd(new StopTimedCmd);
            itsCmdHandler->setTpCmd(itsCmd);
        } break;

        case TBB_UDP:
        {
            UdpCmd *itsCmd(new UdpCmd);
            itsCmdHandler->setTpCmd(itsCmd);
        } break;
