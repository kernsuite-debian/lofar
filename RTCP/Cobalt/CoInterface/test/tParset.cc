...
SUITE(antennaFieldNames) {
  TEST(LBA) {
    ...
    vector<ObservationSettings::AntennaFieldName> antennaFieldNames = ObservationSettings::expandAntennaFieldNames(stations, "LBA_INNER");
  ...
  TEST(HBA0) {
    ...
    vector<ObservationSettings::AntennaFieldName> antennaFieldNames = ObservationSettings::expandAntennaFieldNames(stations, "HBA_ZERO");
  ...
  TEST(HBA1) {
    ...
    vector<ObservationSettings::AntennaFieldName> antennaFieldNames = ObservationSettings::expandAntennaFieldNames(stations, "HBA_ONE");
  ...
  TEST(HBA_DUAL) {
    ...
    vector<ObservationSettings::AntennaFieldName> antennaFieldNames = ObservationSettings::expandAntennaFieldNames(stations, "HBA_DUAL");
  ...
  TEST(HBA_JOINED) {
    ...
    vector<ObservationSettings::AntennaFieldName> antennaFieldNames = ObservationSettings::expandAntennaFieldNames(stations, "HBA_JOINED");
  ...
  SUITE(indices) {
    ...
    TEST(one_present) {
      ...
      sub.push_back(ObservationSettings::AntennaFieldName("CS001","LBA"));
      correct_result.push_back(0);
    ...
    TEST(one_not_present) {
      ...
      full.push_back(ObservationSettings::AntennaFieldName("CS001","LBA"));
      sub.push_back(ObservationSettings::AntennaFieldName("CS002","LBA"));

      CHECK_THROW(ObservationSettings::AntennaFieldName::indices(sub, full), CoInterfaceException);
    }
    ...
    TEST(multi_present) {
      ...
      full.push_back(ObservationSettings::AntennaFieldName("CS001","LBA"));
      full.push_back(ObservationSettings::AntennaFieldName("CS002","LBA"));
      full.push_back(ObservationSettings::AntennaFieldName("CS003","LBA"));
      sub.push_back(ObservationSettings::AntennaFieldName("CS001","LBA"));
      sub.push_back(ObservationSettings::AntennaFieldName("CS002","LBA"));
      sub.push_back(ObservationSettings::AntennaFieldName("CS003","LBA"));
      correct_result.push_back(0);
...
SUITE(stations) {
  ...
  TEST(empty_map) {
    ...
    ps.replace("Observation.Beam[0].subbandList", str(format("[1..%s]") % nrSubbands));
    ...
    for (size_t st = 0; st < 3; st++) {
      ...
      for (size_t sb = 0; sb < nrSubbands; sb++) {
        const size_t nrSlotsPerBoard = ps.settings.nrBitsPerSample == 4  ? 244
                                     : ps.settings.nrBitsPerSample == 8  ? 122
                                     : ps.settings.nrBitsPerSample == 16 ? 61
                                     : 0; // won't happen

        const size_t &board = ps.settings.antennaFields[st].rspBoardMap[sb];
        const size_t &slot  = ps.settings.antennaFields[st].rspSlotMap[sb];

        CHECK_EQUAL(sb, board * nrSlotsPerBoard + slot);
      }
    }
  }
