...
namespace LOFAR
{
  ...
  public:
    ...
    // Write the flags to a set of bytes (set each byte to 1 if included, 0 otherwise)
    void    toByteset(char *ptr, size_t maxSize) const;
  ...
  template <typename T>
  void SparseSet<T>::toByteset(char *ptr, size_t maxSize) const
  {
    memset(ptr, 0, maxSize);

    for (const_iterator it = ranges.begin(); it != ranges.end(); it++) {
      memset(ptr + it->begin, 1, it->end - it->begin);
    }
  }
