...
namespace LOFAR {
  ...
  public:
    ...
    /// Doing this can be useful to avoid frequent resizing, but it can also be used
    /// to make the reservation smaller.
    /// 
    /// It clears the current plan if the size differs from the current reservation.
    ...
    /// 
    /// The data buffer should be filled AFTER making the plan, because
    /// the FFTW plan generator can destroy the data in the buffer.
    /// 
    /// This is the only function that is not thread-safe, so it is
    /// enclosed in a critical section.
    /// 
    /// The 'flags' argument tells how FFTW should create the plan if
    /// there is no wisdom available for the given FFT size.
    /// - FFTW_ESTIMATE is fast, but will only do a rough estimation.
    /// - FFTW_MEASURE will do some actual FFTs to find the best plan.
    /// - FFTW_PATIENT will do more FFTs to find the best plan.
    /// - FFTW_EXHAUSTIVE will do even more FFTs (and take a lot of time).
