...
namespace LOFAR {
  ...
  namespace Cobalt {
    ...
    public:
      ...
      // We use a global lock, as MPI_THREAD_MULTIPLE implementations
      // do so anyway (MPICH2), or are broken (OpenMPI).
      // Hence, we require MPI_THREAD_SERIALIZED.
      bool threadSafe() const { return provided_mpi_thread_support >= MPI_THREAD_SERIALIZED; }
