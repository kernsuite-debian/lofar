...
op = OptionParser(usage = 'usage: python3 %prog [options]', version = '%prog 0.1')
...
def makeRSPVersionGold():
    ...
    time.sleep(3)
    if StationType == International:
        f_log = open('/misc/home/etc/stationtest/gold/rsp_version-int.gold', 'w')
    else:
        f_log = open('/misc/home/etc/stationtest/gold/rsp_version.gold', 'w')
    for line in res:
        print(('Res = ', line))
        f_log.write(line)
...
def CheckSPUStatus():
    ...
    res = cli.command('python3 i2c_spu.py --sub %s --rep 1 -v 11' % (SubRck,))
    res = cli.command('python3 i2c_spu.py --sub sub0,sub1,sub2')
...
def CheckRSPTdI2C():
    ...
    res = cli.command('python3 i2c_td.py --brd %s' % (SubBrd,))
...
def Bist():
    ...
    res = cli.command('python3 verify.py --brd %s --rep 1 -v 21 --te tc/bist.py' % (RspBrd,))
    if debug: print(('res = %s' % res))
    if res.find('wrong') == -1:
...
def PseudoRandomRSPTest():
    ...
    res = cli.command('python3 verify.py --brd %s --fpga blp0,blp1,blp2,blp3 --pol x,y --rep 1 -v 11 --te tc/prsg.py' % (RspBrd,))
    if res.find('wrong') == -1:
...
def RCUHBAModemTest():
    ...
    res = cli.command('python3 verify.py --brd %s --fpga blp0,blp1,blp2,blp3 --rep 1 -v 11 --te tc/hba_client.py --client_access r --client_reg version --data 10' % (RspBrd,))
    if res.find('wrong') == -1:
...
def SerdesRingTestOff():
    ...
    res = cli.command('python3 verify.py --brd %s --rep 1 -v 21 --te tc/serdes.py --diag_sync 0' % (RspBrd,))
    if res.find('wrong') == -1:
...
#       dir_name = './lbadatatest/' #Work directory will be cleaned
    ...
    # init log file
    f_log = open('/opt/stationtest/test/hbatest/LBA_elements.log', 'w')
    f_log.write(' ************ \n \n LOG File for LBA element test \n \n *************** \n')
    f_logfac = open('/opt/stationtest/test/hbatest/LBA_factors.log', 'w')
    f_loglin = open('/opt/stationtest/test/hbatest/LBA_lin.log', 'w')
    f_logdown = open('/opt/stationtest/test/hbatest/LBA_down.log', 'w')    # log number that indicates if LBA antenna is falen over (down)
# initialize data arrays
    ref_data = list(range(0, num_rcu))
    meet_data = list(range(0, num_rcu))
    meet_data_left = list(range(0, num_rcu))
    meet_data_right = list(range(0, num_rcu))
    meet_data_down = list(range(0, num_rcu))
    os.chdir(dir_name)
