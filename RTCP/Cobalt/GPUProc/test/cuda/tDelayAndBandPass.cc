...
const unsigned NR_DELAYS = NR_STATIONS*2; // if changed, take care of delayInidices initialisation as well
               ...
               bool transpose)
{
  ...
  gpu::DeviceMemory devStationIndices (ctx, delayIndices.size());
  ...
  stream->writeBuffer(devStationIndices,    delayIndices);
  ...
  if (transpose) {
    ...
    gpu::Grid globalWorkSize(NR_SAMPLES_PER_CHANNEL / 16,
                           NR_CHANNELS == 1 ? 1 : NR_CHANNELS / 16,
                           NR_STATIONS);
    gpu::Block localWorkSize(16, NR_CHANNELS == 1 ? 1 : 16, 1);
    stream->launchKernel(kfunc, globalWorkSize, localWorkSize);
  }
                         ...
                         float bandPassFactor)
{
  ...
  MultiDimArrayHostBuffer<unsigned, 1> delayIndices(boost::extents
                                                   [NR_STATIONS],
                                                   ctx);
  ...
  for (size_t i = 0; i < delayIndices.num_elements(); i++) {
    delayIndices.origin()[i] = i * 2; // use elements in delays arrays beyond NR_STATIONS
  }
