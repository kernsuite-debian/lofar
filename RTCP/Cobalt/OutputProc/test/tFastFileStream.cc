...
{
public:
  TempFile( const string &dirname = ".") /* note: /tmp could be using tmpfs, which does not support O_DIRECT */
  {
    char templ[1024];
    snprintf(templ, sizeof templ, "%stFastFileStream_tmpXXXXXX", dirname.c_str()); /* ./$test_tmp* is cleaned up by assay */
