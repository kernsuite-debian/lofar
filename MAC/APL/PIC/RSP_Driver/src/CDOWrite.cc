...
void CDOWrite::setup_udpip_header(uint32 l_srcip, uint32 l_dstip, uint16 l_dstport)
{
  ...
  LOG_INFO(str(format("IP Header checksum: %04x (calculated over %d bytes)") % static_cast<uint16>(m_udpip_hdr.ip.hdrchksum) % sizeof m_udpip_hdr.ip));
	...
	m_udpip_hdr.udp.dstport     = htons(l_dstport);
...
uint16 CDOWrite::compute_ip_checksum(const CDOWrite::UDPIPType &header)
{
	...
	const uint16* addr16 = static_cast<const uint16*>(addr);
	while( count > 1 )  {
		/*  This is the inner loop */
                LOG_INFO_STR("IP Checksum calculation: rolling sum = " << sum << " next 16-bit sample is " << *addr16);
		sum += *addr16++;
		count -= 2;
	}
	...
	if( count > 0 ) {
                LOG_INFO_STR("IP Checksum calculation: rolling sum = " << sum << " next 8-bit sample is " <<  reinterpret_cast<const uint8*>(*addr16));
		sum += * reinterpret_cast<const uint8*>(addr16);
  }
	...
	while( sum >> 16 ) {
        ...
        LOG_INFO_STR("IP Checksum calculation: sum = " << sum << ", ~sum = " << (~sum) << ", (~sum) & 0xffff = " << ((~sum) & 0xffff));
...
//		LOG_INFO_STR("CDO: splitter[" << getBoardId() << "] is " << (Cache::getInstance().getBack().isSplitterActive() ? "ON" : "OFF"));
		...
		if (output_lane >= 0) {
			...
			char srcmacfmt[64], srcmac[64], dstmac[64], dstip[64], srcip[64], dstport[64];
      ...
      try {
        ...
        l_dstport = BASEUDPPORT + getBoardId();
			...
			LOG_INFO(formatString("CDO: RSP=%d, lane=%d(%d), src=[%s,%s], dst=[%s,%s,%d]",
								...
								GET_CONFIG_STRING(dstmac), GET_CONFIG_STRING(dstip), l_dstport));
			...
			// setup UDP/IP header
			setup_udpip_header(l_srcip, l_dstip, l_dstport);
