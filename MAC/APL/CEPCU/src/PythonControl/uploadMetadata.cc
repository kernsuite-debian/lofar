...
{
	if (argc != 3) {
		cout << "Syntax: " << argv[0] << " treeID metadatafile" << endl;
		return (-1);
	}

        int treeID = atoi(argv[1]);
        string parsetFilename = argv[2];

	// read parameterset
	ParameterSet	metadata;
	metadata.adoptFile(parsetFilename);

        // read database credentials
        ConfigLocator CL;
        string  filename = CL.locate("uploadMetadata.conf");
        LOG_DEBUG_STR("Trying to read database information from file " << filename);
        ParameterSet  otdbconf;
        otdbconf.adoptFile(filename);

        string dbUser     = otdbconf.getString("uploadMetadata.OTDBusername");
        string dbPassword = otdbconf.getString("uploadMetadata.OTDBpassword");
        string dbHost     = otdbconf.getString("uploadMetadata.OTDBhostname");
        string dbPort     = otdbconf.getString("uploadMetadata.OTDBport");
        string dbDatabase = otdbconf.getString("uploadMetadata.OTDBdatabase");


	// Connect to KVT logger
	OTDBconnection	conn(dbUser, dbPassword, dbDatabase, dbHost, dbPort);
	if (!conn.connect()) {
		cerr << "Cannot connect to database " << dbDatabase << " as " << dbUser << " on machine " << dbHost << ":" << dbPort << endl;
		return (-2);
	}
	cout << "Connected to database " << dbDatabase << " as " << dbUser << " on machine " << dbHost << ":" << dbPort << endl;
