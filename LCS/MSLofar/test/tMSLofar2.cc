//# tMSLofar2.cc : this program tests the MSLofar class
//# Copyright (C) 2011
...
uInt tConstructors(const String& msName)
{
    ...
    // make non MS table
    {
      SetupNewTable newtab("tMSLofar2_tmp.badmsTable",
			   "badTD",Table::New);
      Table tab(newtab);
    }
    ...
    Bool thrown=False;
    try {
      MSLofar badms("tMSLofar2_tmp.badmsTable");
    } catch (AipsError x) {
      thrown = True;
    } 
    if (!thrown) errCount++;
    ...
    MSLofar tabRefMS(tab);

    // try the same with a bad table
    thrown=False;
    try {
      Table badtab("tMSLofar2_tmp.badmsTab;e","");
      MSLofar badms(badtab);
    } catch (AipsError x) {
      thrown = True;
    } 
    if (!thrown) errCount++;
    ...
    Table badtab("tMSLofar2_tmp.badmsTable",Table::Delete);
    TableDesc("badTD",TableDesc::Delete);

    // finally, as a const MS referencing that MS
    const MSLofar constRefMS(tabRefMS);
	...
	TableDesc td2(MSFeed::requiredTableDesc(),"badAntTD","",TableDesc::New);
      ...
      SetupNewTable newtab1("tMSLofar2_tmp.msant2","antTD",Table::New);
      {
	MSAntenna msant(newtab1);
      }
      MSAntenna msant2("tMSLofar2_tmp.msant2","antTD",Table::Old);
      
      // try an invalid tableDesc
      Bool thrown=False;
      try {
	MSAntenna msant2b("tMSLofar2_tmp.msant2","badAntTD",Table::Old);
	...
	thrown = True;
      ...
      SetupNewTable newtab2("tMSLofar2_tmp.msant3",
      ...
      thrown = False;
      try {
	SetupNewTable newtab("tMSLofar2_tmp.msant3b",
	...
	tab.markForDelete();
      ...
      SetupNewTable newtab3("tMSLofar2_tmp.msantTable",
			    "antTD",Table::New);
      Table tab(newtab3);
      tab.markForDelete();
      ...
      thrown = False;
      try {
	SetupNewTable newtab4("tMSLofar2_tmp.badmsantTable",
			      "badAntTD",Table::New);
	Table tab(newtab4);
	tab.markForDelete();
	...
	thrown = True;
      ...
      MSAntenna msant5("tMSLofar2_tmp.msantTable",Table::Old);

      // try invalid table
      thrown = False;
      try {
	{
	  Table tab("tMSLofar2_tmp.badmsantTable","badAntTD",Table::New);
	}
	MSAntenna msant5b("tMSLofar2_tmp.badmsantTable");
      } catch (AipsError x) {
	thrown = True;
      } 
      if (!thrown) errCount++;
...
uInt tReferenceCopy(const String& msName, const String& refMSName)
{
    ...
    MSLofar ms(msName,Table::Update);
    {
    ...
    MSLofar refCopyMS = ms.referenceCopy(refMSName, 
						writableColumn);
...
uInt tSetupNewTabError()
{
    ...
    if (!thrown) {
	cerr << "MSLofar(SetupNewTable &, uInt) " 
...
uInt tDestructorError(const String& sdmsName)
{
    ...
    if (!thrown) {
	cerr << "~MSLofar() should have thrown an exception" << endl;
	errCount++;
    }
...
int main() {
  ...
  try {
    ...
    String msName = "tMSLofar2_tmp.Table";
    String refMSName = "tMSLofar2_tmp.Ref-Table";
    ...
    if (errCount > 0) {
	cout << "tMSLofar2 ends with " << errCount << " errors." << endl;
    } else {
	cout << "tMSLofar2 ends successfully" << endl;
    }
