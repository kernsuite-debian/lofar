...
/**
  ...
  *
  * With the new LCU's we need to use the new Blitz 0.10 version that has a change in array use.
  * So for now a Blitz010 bool is used to check if it is the new Blitz or not. 
  * The files for the new Blitz version are indicated with the line :
  * 
  * # Blitz010 formatted
  *
  ...
  */
  ...
  global int nr_HBA=0;
  global int nr_LBA=0;
  global int nr_POWECS=0;
  global string stationIP = "";
...
{ 
  ...
  while (dynlen(dynStr_fileContent) > 3  && index < dynlen(dynStr_fileContent)) {
    ...
    if (strlen(str1) <= 0 || str1[0] == " " || str1[0] == "#") {
      if (strpos(dynStr_fileContent[index-1],"Blitz-0.10 formatted") >= 0) {
        DebugN("Blitz: ", str2);
        Blitz010=true;
      }
      continue;
  ...
  for (index=1;index <= dynlen(dynStr_fileContent);index++) {
    ...
    if (strpos(dynStr_fileContent[index],"RS.STATION_IP")>-1) {
      dyn_string value = strsplit(dynStr_fileContent[index],"=");
      if (dynlen(value) > 1) {
        stationIP = value[2];
        dpSet("LOFAR_PIC_StationInfo.stationIP",stationIP);
      }
    }
    ...
    if (strpos(dynStr_fileContent[index],"RS.N_POWECS")>-1) {
      dyn_string value = strsplit(dynStr_fileContent[index],"=");
      if (dynlen(value) > 1) {
        nr_POWECS = value[2];
        dpSet("LOFAR_PIC_StationInfo.nrOfPowerUnits",nr_POWECS);
      }
    }
  ...
  // set POWEC IP address for all available POWECS.
  dyn_string ipparts = strsplit(stationIP,".");
  string ip = ipparts[1] + "." + ipparts[2] + "." + ipparts[3] + ".";

  dpSet("_2_SNMPAgent_1.Access.IPAddress",ip+"5");
  dpSet("LOFAR_PIC_POWEC0.nrOfModules:_address.._active",1);
  dpSet("LOFAR_PIC_POWEC0.voltage:_address.._active",1);
  dpSet("LOFAR_PIC_POWEC0.current:_address.._active",1);
  dpSet("LOFAR_PIC_POWEC0.temperature:_address.._active",1);
  dpSet("LOFAR_PIC_POWEC0.OK:_address.._active",1);
  if (nr_POWECS == 2) {
    dpSet("_2_SNMPAgent_2.Access.IPAddress",ip+"7");
    dpSet("LOFAR_PIC_POWEC1.nrOfModules:_address.._active",1);
    dpSet("LOFAR_PIC_POWEC1.voltage:_address.._active",1);
    dpSet("LOFAR_PIC_POWEC1.current:_address.._active",1);
    dpSet("LOFAR_PIC_POWEC1.temperature:_address.._active",1);
    dpSet("LOFAR_PIC_POWEC1.OK:_address.._active",1);
  } else if (nr_POWECS == 1) {
    dpSet("LOFAR_PIC_POWEC1.nrOfModules:_address.._active",0);
    dpSet("LOFAR_PIC_POWEC1.voltage:_address.._active",0);
    dpSet("LOFAR_PIC_POWEC1.current:_address.._active",0);
    dpSet("LOFAR_PIC_POWEC1.temperature:_address.._active",0);
    dpSet("LOFAR_PIC_POWEC1.OK:_address.._active",0);      
  }  
}
