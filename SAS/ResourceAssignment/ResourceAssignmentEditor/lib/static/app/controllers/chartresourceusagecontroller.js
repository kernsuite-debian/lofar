...
chartResourceUsageControllerMod.controller('ChartResourceUsageController', ['$scope', 'dataService', function($scope, dataService) {
    ...
    $scope.$watch('dataService.lofarTime', function() {
        var lofarTime = $scope.dataService.lofarTime;
        if(lofarTime.getSeconds() % 10 == 0) {
            $scope.$evalAsync(updateChartLofarTime);
        }
    });

    function updateChartDataAsync() {
        $scope.$evalAsync(updateChartData);
    };

    function updateChartData() {
        if(!$scope.$parent.enabled) {
            return;
        }
        ...
        var resourceUsagesForSelectedResource = $scope.dataService.resourceUsagesForSelectedResource;
        ...
        var selectedResource = resourceDict[selected_resource_id];

        if(!selectedResource || numResources == 0 || !resourceUsagesForSelectedResource) {
            $scope.chartSeries.splice(0, $scope.chartSeries.length);
        ...
        //set title, axis etc
        $scope.chartConfig.title.text = selectedResource.name;
        $scope.chartConfig.yAxis.title.text = selectedResource.unit;
        ...
        //first scan of all statuses and timestamps in usages for this selectedResource
        var statuses = [];
        var timestamps = [];
        for(var status in resourceUsagesForSelectedResource) {
            statuses.push(status);

            var usages = resourceUsagesForSelectedResource[status];
            for(var usage of usages) {
                timestamps.push(usage.as_of_timestamp);
            }
        }
        ...
        if(timestamps.length > 0) {
            ...
            // loop again over the  usages for this selectedResource
            ...
            for(var status of ['conflict', 'tentative', 'claimed']) {
                if(!resourceUsagesForSelectedResource.hasOwnProperty(status))
                    continue;
                ...
                while(t_idx < t_length) {
                    ...
                    if(u_idx < u_length-1 && timestamp >= usages[u_idx+1].as_of_timestamp) {
                        u_idx += 1;
                    }
                    ...
                    if(u_idx < u_length) {
                        ...
                        var u_min_timestamp = usages[0].as_of_timestamp;
                        var u_max_timestamp = usages[u_length-1].as_of_timestamp;
            ...
            //plot area for selectedResource misc_used capacity
            ...
            var misc_used_capacity = selectedResource.misc_used_capacity;
            ...
            //plot horizontal line for selectedResource total capacity
            ...
            series.data = [[timestamps[0].getTime(), selectedResource.total_capacity],
                           [timestamps[timestamps.length-1].getTime(), selectedResource.total_capacity]]
            expectedSeriesNames.push('total capacity');

            //plot horizontal line for selectedResource used capacity
            ...
            series.visible = self.seriesVisibilityCache.hasOwnProperty(series.name) ? self.seriesVisibilityCache[series.name] : true;
            series.data = [[timestamps[0].getTime(), selectedResource.used_capacity],
                           [timestamps[timestamps.length-1].getTime(), selectedResource.used_capacity]]
