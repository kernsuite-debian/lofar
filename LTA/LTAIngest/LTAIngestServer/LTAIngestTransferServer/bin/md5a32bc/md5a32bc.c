 ...
 **********************************************************************
 ** md5a32bc.c                                                       **
 ** Compute md5, adler32 and byte count while transfering data from  **
 ** stdin to stdout. Results are printed on stderr.                  **
 **                                                                  **
 ** md5 methods are copied from                                      **
...
MD5_CTX *mdContext;
{
  ...
  /* Set adler32 init value to one
   */
  mdContext->adler32 = 1;

  mdContext->byte_count = 0;
}
...
int main (int argc, char *argv[])
{
    ...
    while (1)
    {
        num_bytes_read = fread (data, 1, BUFFSIZE, stdin);

        if(num_bytes_read == 0)
            break;

        mdContext.adler32=adler32(mdContext.adler32, data, num_bytes_read);
